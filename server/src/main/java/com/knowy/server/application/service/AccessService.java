...
public class AccessService {
    ...
    public PublicUser registerNewUser(UserDto userDto) throws InvalidUserException {
		...
		ProfileImage profileImage = profileImageRepository.findById(1).orElseThrow();
        PublicUser publicUser = new PublicUser(null, userDto.getUsername(), profileImage);
        PrivateUser privateUser = new PrivateUser(publicUser, new Email(), new Password());

        publicUser = privateUserRepository.update(privateUser);
